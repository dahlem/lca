    ...
    : m_opt_desc(new po::options_description("Options"))
{
  ...
  opt_io.add_options()
      ...
      (EULER_CIRCUIT.c_str(), po::value <std::string>()->default_value(""), "Filename of the Euler Circuit.")
      ...
      (QUERY_SET.c_str(), po::value <std::string>()->default_value(""), "Filename of the query set to compute pair-wise LCAs over.")
      ;
...
int CL::parse(int argc, char *argv[], args_t &p_args)
{
  ...
  if (vm.count(EULER_CIRCUIT.c_str())) {
    p_args.euler_circuit = vm[EULER_CIRCUIT.c_str()].as <std::string>();
    if (!fs::exists(p_args.euler_circuit)) {
      std::cerr << "The filename " << p_args.euler_circuit << " with the Euler Circuit does not exist!" << std::endl;
      return EXIT_FAILURE;
    }
  }
  ...
  if (vm.count(QUERY_SET.c_str())) {
    p_args.query_set = vm[QUERY_SET.c_str()].as <std::string>();
    if (!fs::exists(p_args.euler_levels)) {
      std::cerr << "The filename " << p_args.query_set << " containing the query set does not exist!" << std::endl;
      return EXIT_FAILURE;
    }
  }
